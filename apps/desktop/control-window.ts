...
type ControlOsWindow = {
	bind: (name: string, handler: (...args: unknown[]) => unknown) => void;
};

export type BindControlWindowOptions = {
	reopenPreview: (url: string) => void;
};

export function controlPageHtml(defaultRepo: string): string {
	return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<main>
  ...
  <section>
    ...
    <input id="repo" value="${defaultRepo}" />
...
<script>
const DEFAULT_REPO = ${JSON.stringify(defaultRepo)};
const scenarios = [
  {
    id: "happy",
    name: "Happy path",
...
export function bindControlWindow(
	win: ControlOsWindow,
	session: ProjectSession,
	opts: BindControlWindowOptions,
): void {
	...
	win.bind("reopenPreview", () => {
		...
		if (snap.previewUrl) opts.reopenPreview(snap.previewUrl);
		return snap;
	});
}
